...
---------------------------------------------------------   List processing  ------------------------------------------------------------------------------------------------------
            ...
            thin2()         remove list duplicates                          list of lists                                               list of lists   OpenMusic
            ...
            indexmap2()     index list detecting list repetition            list of lists                                               list            OpenMusic
...
---------------------------------------------------------   Machine learning  ------------------------------------------------------------------------------------------------------
            markov.build()  build a markov model of nth-order               dataset, order                                              list of lists   FTH
...
// list markov.build.old(list data_, float order_) {
//     float i_, j_, k_
//     list matrix_, postrans_, ngrams_, dict_
//     ngrams_ = frames(data_, order_, 1) // sequence of ngrams
//     dict_ = thin2(ngrams_) // dictionary of unique states
...
//             }
//         }
//         postrans_[i_] = pstates_ // include transition row for each state
//     }
//     // populate a list (matrix_) with lists of transition probabilities for each unique ngram
//     for (i_ = 0; i_ < len(postrans_); i_ += 1) {
//         list countList_, current_
//         countList_ = {}
//         current_ = postrans_[i_]
//         for (j_ = 0; j_ < len(dict_); j_ += 1) {
//             float count_, k_, m_
//             count = 0
//             list ngr_
//             ngr_ = dict_[j_]
//             for (k_ = 0; k_ < len(current_); k_ += 1) {
//                 for (m_ = 0; m_ < len(ngr_); m_ += 1) {
//                     if (compare(current_[k_], ngr_)) {
//                         count_ += 1
//                     }
//                 }
//             }
//             countList_[j_] = count_
//         }
//         if (reduce(countList_) == 0) { // if no possible transitions
//             float c_
//             for (c_ = 0; c_ < len(countList_); c_ += 1) { // populate with 1
//                 countList_[c_] = 1
//             }
//         }
//         countList_ = countList_ / reduce(countList_) // get probabilities as decimals
...
list markov.run(list dict_, list m_, float times_) {
    float i_, j_, k_, s_
    list pl_, out_, e_
    s_ = 0 // initial state index
    k_ = 1 // output index
